...
import { Plus, Trash2, WifiOff, QrCode, RefreshCw, LogOut, Send, Settings2 } from "lucide-react";
...
import {
  ...
  sendMessage, setInstanceProxy, testInstanceProxy, type Instance,
} from "@/lib/api";
...
export function Instances() {
  ...
  // Proxy dialog state
  const [proxyTarget, setProxyTarget] = useState<Instance | null>(null);
    ...
    <div className="space-y-6">
              ...
              <Card key={inst.id}>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    ...
                    <Button size="sm" variant="outline" title="Proxy settings" onClick={() => setProxyTarget(inst)}>
                  ...
                  {inst.proxyUrl && (
                    <p className="text-xs text-muted-foreground font-mono truncate pt-1" title={inst.proxyUrl}>
                      Proxy: {inst.proxyUrl.replace(/:[^@]+@/, ":***@")}
                    </p>
                  )}
                </CardContent>
              </Card>
      ...
      {proxyTarget && (
        <ProxyDialog
          ...
          onSaved={() => { qc.invalidateQueries({ queryKey: ["instances"] }); setProxyTarget(null); }}
        />
      )}
    </div>
  );
...
function ProxyDialog({ instance, onClose, onSaved }: { instance: Instance; onClose: () => void; onSaved: () => void }) {
  const [url, setUrl] = useState(instance.proxyUrl ?? "");
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await setInstanceProxy(instance.id, url.trim() || null);
      toast({ title: url.trim() ? "Proxy saved" : "Proxy cleared" });
      onSaved();
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error ?? "Failed to save proxy";
      toast({ title: "Error", description: msg, variant: "destructive" });
    } finally {
      setSaving(false);
    }
  ...
  async function handleTest() {
    ...
    if (!trimmed) { toast({ title: "Enter a proxy URL first", variant: "destructive" }); return; }
    setTesting(true);
    setTestResult(null);
    try {
      const res = await testInstanceProxy(instance.id, trimmed);
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Proxy — {instance.slug}</DialogTitle>
          <DialogDescription>
            Route this instance's WhatsApp connection through a proxy. Takes effect on next connect.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSave} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              placeholder="socks5://user:pass@host:1080 or http://host:3128"
              value={url}
              onChange={(e) => { setUrl(e.target.value); setTestResult(null); }}
            />
            <p className="text-xs text-muted-foreground">
              Supports <code>socks5://</code>, <code>socks4://</code>, <code>http://</code>, <code>https://</code>. Leave blank to disable.
            </p>
          </div>

          {testResult && (
            <div className={`rounded-md px-3 py-2 text-sm ${testResult.success ? "bg-green-50 text-green-800 border border-green-200" : "bg-red-50 text-red-800 border border-red-200"}`}>
              {testResult.success
                ? <span>✓ Connected — exit IP: <strong>{testResult.ip}</strong></span>
          ...
          <div className="flex gap-2">
            ...
            <Button type="submit" disabled={saving || testing} className="flex-1">
              {saving ? "Saving…" : "Save"}
            </Button>
            {instance.proxyUrl && (
              <Button
                ...
                onClick={async () => {
                  setSaving(true);
                  try {
                    await setInstanceProxy(instance.id, null);
