...
import { enqueueCampaign, cancelCampaign, getQueueInfo, DEFAULT_OPTIONS, DEFAULT_GROUP_OPTIONS, MAX_RECIPIENTS_HARD_CAP } from "../services/bulkSender.js";
...
router.post("/:campaignId/cancel", async (req: Request, res: Response) => {
  const campaign = await db("bulk_campaigns")
    ...
    .first();
  if (!campaign) return res.status(404).json({ error: "Campaign not found" });
  if (!["pending", "queued", "running", "waiting"].includes(campaign.status)) {
